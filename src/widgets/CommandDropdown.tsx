/**
 * CommandDropdown — autocomplete dropdown for slash commands or any item list.
 *
 * Shows items vertically with a highlighted selection indicator.
 * Displays "..." overflow indicators when items exceed the visible window.
 *
 * @module
 */
...
export interface CommandDropdownProps {
  ...
  /** Whether the dropdown is focused and captures keyboard input. */
  isFocused?: boolean;
...
export const CommandDropdown = React.memo(function CommandDropdown(rawProps: CommandDropdownProps): React.ReactElement | null {
  ...
  const props = usePluginProps("CommandDropdown", rawProps as unknown as Record<string, unknown>) as unknown as CommandDropdownProps;
  const {
    items,
    selectedIndex = 0,
    maxVisible = 6,
  ...
  // Compute filtered items and match indices
  ...
  let start = 0;
  if (total > maxVisible) {
    start = Math.max(0, Math.min(activeIndex - Math.floor(maxVisible / 2), total - maxVisible));
  }
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      ...
      {visibleItems.map((entry, i) => {
        ...
        // Compute unselected padding to match indicator width
