/**
 * OperationTree — hierarchical operation progress display.
 *
 * Renders a tree of operations with status icons, labels, details,
 * and durations. Running nodes animate with a spinner using
 * imperative requestRender() — no React setState involved.
 */
...
function collectRunningRefs(
  ...
  for (const node of nodes) {
    if (node.status === "running") {
      // Ensure a ref exists for this node
      if (!refs.has(node.id)) {
...
export const OperationTree = React.memo(function OperationTree(rawProps: OperationTreeProps): React.ReactElement {
  ...
  const props = usePluginProps("OperationTree", rawProps as unknown as Record<string, unknown>) as unknown as OperationTreeProps;
  ...
  // Collect refs for running nodes
