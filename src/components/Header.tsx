/**
 * Header — full-width header bar with title and optional subtitle.
 *
 * Renders a prominent header with thick border lines above and below.
 * Title is bold and colored, subtitle is dim.
 *
 * Features:
 *   - right: right-aligned content in the title row (via flex spacer)
 *   - showBorder: optionally hide borders (default true)
 */
...
export interface HeaderProps extends StormLayoutStyleProps {
  title: string;
  subtitle?: string;
...
const BORDER_CHARS: Record<"single" | "double" | "round", string> = {
  single: "\u2500", // ─
  double: "\u2501", // ━
  round: "\u2500",  // ─ (thin line for Quiet Conductor)
};
...
export const Header = React.memo(function Header(rawProps: HeaderProps): React.ReactElement {
  ...
  const props = usePluginProps("Header", rawProps as unknown as Record<string, unknown>) as unknown as HeaderProps;
