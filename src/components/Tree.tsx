 ...
 *
 * Renders nodes with proper tree connector lines (│, ├──, └──) matching
 * DirectoryTree's style. Supports optional icons, virtual scrolling,
...
function collectVisibleKeys(nodes: TreeNode[], depth: number): string[] {
  if (depth >= MAX_TREE_DEPTH) return [];
  const keys: string[] = [];
...
function findNode(nodes: TreeNode[], key: string, depth: number): TreeNode | undefined {
...
export const Tree = React.memo(function Tree(rawProps: TreeProps): React.ReactElement {
  ...
  const props = usePluginProps("Tree", rawProps as unknown as Record<string, unknown>) as unknown as TreeProps;
  const {
    nodes,
    onToggle,
  ...
  for (let i = visibleStart; i < visibleEnd; i++) {
    ...
    // Build tree connector prefix (matching DirectoryTree style)
    let prefix = "";
