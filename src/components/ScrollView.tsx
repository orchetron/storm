/**
 * ScrollView — scrollable container with hit-tested mouse AND keyboard scroll.
 *
 * Registers with focusManager for hit-testing. Mouse scroll events are
 * routed by render.ts based on mouse position — only the ScrollView
 * under the cursor scrolls. Keyboard scroll (PgUp/PgDown, Shift+arrows)
 * is routed to the "active" ScrollView — the one most recently hit-tested
 * by mouse scroll, or the sole ScrollView if only one exists.
 *
 * Inside a Modal/dialog focus trap, scroll keys are forwarded by the
 * trap handler directly to the active ScrollView's callbacks.
 */
...
export const ScrollView = React.memo(function ScrollView(rawProps: ScrollViewProps): React.ReactElement {
  const props = usePluginProps("ScrollView", rawProps as unknown as Record<string, unknown>) as unknown as ScrollViewProps;
  const {
    children,
    scrollSpeed = 3,
    stickToBottom = false,
  ...
  const scrollStateRef = useRef<ScrollState>(undefined as unknown as ScrollState);
  ...
  // Store handlers in refs so eager registrations always use latest
  const handleScrollRef = useRef(handleScroll);
  handleScrollRef.current = handleScroll;
  const handleHScrollRef = useRef(handleHScroll);
  handleHScrollRef.current = handleHScroll;

  // Store horizontalScroll in a ref so the key handler reads the latest value
