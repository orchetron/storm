/**
 * Alert — boxed attention message with closable and action support.
 *
 * Renders a bordered box with border color based on type (success, warning,
 * error, info). Optionally shows a bold colored title above the children.
 *
 * Features:
 *   - closable: show "x" and handle Escape to dismiss
 *   - action: render an action link after the message
 */
...
const AlertBase = React.memo(function Alert(rawProps: AlertProps): React.ReactElement {
  const colors = useColors();
  const props = usePluginProps("Alert", rawProps as unknown as Record<string, unknown>) as unknown as AlertProps;
  const personality = usePersonality();
  const {
    children,
    type = "info",
