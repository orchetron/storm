/**
 * DefinitionList — term and definition pairs.
 *
 * Renders a list of term/definition entries.
 * Supports stacked layout (term above, definition indented below)
 * and inline layout (term and definition on the same line).
 * In inline mode, auto-aligns all term widths to the widest term.
 * Supports optional separator between items.
 */
...
export const DefinitionList = React.memo(function DefinitionList(rawProps: DefinitionListProps): React.ReactElement {
  const colors = useColors();
  const props = usePluginProps("DefinitionList", rawProps as unknown as Record<string, unknown>) as unknown as DefinitionListProps;
  const {
    items,
    termColor = colors.brand.primary,
    layout = "stacked",
    separator,
  } = props;
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    elements.push(
      React.createElement(
        "tui-box",
        { key: `item-${index}`, flexDirection: "column" },
        // Term line
        stackedTermElement,
