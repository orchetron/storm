 ...
 *
 * Renders a month grid with day-of-week headers (Su Mo Tu We Th Fr Sa).
 * Selected day is bold+inverse. Arrow keys navigate days and call onSelect.
 *
 * Features:
 * - Date range highlighting via rangeStart / rangeEnd props
 * - Disabled dates via disabledDates predicate
 * - Configurable week start (Sunday or Monday) via weekStartsOn prop
 */
...
const CalendarBase = React.memo(function Calendar(rawProps: CalendarProps): React.ReactElement {
  const colors = useColors();
  const props = usePluginProps("Calendar", rawProps as unknown as Record<string, unknown>) as unknown as CalendarProps;
  const {
    year,
    month: rawMonth,
  ...
  // Compute date range for highlighting
  ...
  for (let d = 1; d <= daysInMonth; ) {
    ...
    // Determine last day of this week row
