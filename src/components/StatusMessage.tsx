/**
 * StatusMessage — inline status message with icon.
 *
 * Shows a type-appropriate icon (check, warning, cross, info) followed
 * by an optional bold title and the message text in a row layout.
 *
 * Supports an optional collapsible `detail` section toggled via Enter
...
export const StatusMessage = React.memo(function StatusMessage(rawProps: StatusMessageProps): React.ReactElement {
  const colors = useColors();
  const props = usePluginProps("StatusMessage", rawProps as unknown as Record<string, unknown>) as unknown as StatusMessageProps;
  ...
  if (title) {
    children.push(
      React.createElement(
        "tui-text",
        { color: resolvedColor, bold: resolvedBold, ...(dim !== undefined ? { dim } : {}), key: "title" },
        `${title} `,
      ),
    );
  }
  ...
  children.push(
    React.createElement(
      "tui-text",
      { color: colors.text.primary, ...(dim !== undefined ? { dim } : {}), key: "message" },
      message,
    ),
  );
  ...
  if (detail) {
    ...
    children.push(
      React.createElement(
        "tui-text",
        { color: colors.text.dim, dim: true, key: "toggle" },
        `  ${toggleIcon} Details`,
      ),
    );
  }
  ...
  const mainRow = React.createElement(
    "tui-box",
    { flexDirection: "row", key: "main" },
    ...children,
  );

  // If detail is present and expanded, show detail text below
  if (detail && expandedRef.current) {
    const detailRow = React.createElement(
      "tui-box",
      { key: "detail", paddingLeft: 2, marginTop: 0 },
      React.createElement(
        "tui-text",
