/**
 * GradientBorder — box with gradient-colored border.
 *
 * Renders children inside a box with round border characters (╭╮╰╯─│)
 * where each border character gets an interpolated color transitioning
...
export const GradientBorder = React.memo(function GradientBorder(rawProps: GradientBorderProps): React.ReactElement {
  const themeColors = useColors();
  const props = usePluginProps("GradientBorder", rawProps as unknown as Record<string, unknown>) as unknown as GradientBorderProps;
  const personality = usePersonality();
  const defaultColors: [string, string] = [themeColors.brand.primary, themeColors.brand.glow];
  const {
    children,
  ...
  topElements.push(
    React.createElement("tui-text", { key: "tl", color: colorAt(0) }, "\u256D"),
  );
  for (let i = 0; i < innerWidth; i++) {
    topElements.push(
      React.createElement(
        "tui-text",
        { key: `t${i}`, color: colorAt(1 + i) },
        "\u2500",
      ),
    );
  }
  topElements.push(
    React.createElement(
      "tui-text",
      { key: "tr", color: colorAt(1 + innerWidth) },
      "\u256E",
    ),
  );
  ...
  // ── Bottom border row: ╰──...──╯ ──────────────────────────────────
  const botElements: React.ReactElement[] = [];
  const botStart = totalChars - totalWidth;
  botElements.push(
    React.createElement(
      "tui-text",
      { key: "bl", color: colorAt(botStart) },
      "\u2570",
    ),
  );
  for (let i = 0; i < innerWidth; i++) {
    botElements.push(
      React.createElement(
        "tui-text",
        { key: `b${i}`, color: colorAt(botStart + 1 + i) },
        "\u2500",
      ),
    );
  }
  botElements.push(
    React.createElement(
      "tui-text",
      { key: "br", color: colorAt(totalChars - 1) },
      "\u256F",
    ),
  );

  const botRow = React.createElement(
    "tui-box",
    { key: "bot", flexDirection: "row" },
    ...botElements,
  );
