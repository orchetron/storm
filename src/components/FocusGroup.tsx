/**
 * FocusGroup — groups focusable children and manages focus within.
 *
 * Two modes of operation:
 *
 * 1. **Arrow navigation** (default): Wraps children in a container that traps
 *    keyboard navigation (Up/Down or Left/Right) within the group. Useful for
 *    toolbars, menu bars, and other composite widgets.
 *
 * 2. **Focus trap** (`trap={true}`): Restricts Tab/Shift+Tab cycling to only
 *    the focusable elements registered with this group's ID. Useful for modals,
 *    dialogs, and overlays that must prevent focus from escaping.
 *    On mount, calls `focusManager.trapFocus(groupId)`.
 *    On unmount, calls `focusManager.releaseFocus()`.
 */
...
export const FocusGroup = React.memo(function FocusGroup(rawProps: FocusGroupProps): React.ReactElement {
  const props = usePluginProps("FocusGroup", rawProps as unknown as Record<string, unknown>) as unknown as FocusGroupProps;
  const {
    children,
    id,
    trap = false,
