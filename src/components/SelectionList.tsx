/**
 * SelectionList — multi-select checklist with keyboard navigation.
 *
 * Renders a vertical list with checkboxes. Up/Down navigate,
 * Space toggles selection, A selects all, N deselects all.
 * Shift+Up/Shift+Down for range selection.
 * Shows "N of M selected" indicator at the bottom.
 * Uses useRef for highlight index.
 *
 * Features:
 * - Type-to-filter: typing filters visible items by label (case-insensitive)
 * - Shows "X of Y items" count reflecting filtered vs total
 * - Selection state preserved for filtered-out items
...
export interface SelectionListProps extends StormLayoutStyleProps {
  ...
  renderItem?: (item: SelectionListItem, state: { isHighlighted: boolean; isSelected: boolean }) => React.ReactNode;
}

export const SelectionList = React.memo(function SelectionList(rawProps: SelectionListProps): React.ReactElement {
  const colors = useColors();
  const personality = usePersonality();
  const props = usePluginProps("SelectionList", rawProps as unknown as Record<string, unknown>) as unknown as SelectionListProps;
  const {
    items,
    selectedValues,
    onChange,
  ...
  const getFiltered = (itms: Array<{ label: string; value: string }>, filter: string) => {
    if (!filter) return itms;
  ...
  const handleInput = useCallback((event: KeyEvent) => {
    ...
    if (event.key === "up" && event.shift) {
      ...
      if (currentItem) selSet.add(currentItem.value);
      highlightRef.current =
        highlightRef.current > 0
          ? highlightRef.current - 1
          : filtered.length - 1;
      const nextItem = filtered[highlightRef.current];
      if (nextItem) selSet.add(nextItem.value);
      cb([...selSet]);
      ...
      if (props.renderItem) {
        rows.push(
          React.createElement(
            ...
            props.renderItem(item as SelectionListItem, { isHighlighted, isSelected: isChecked }),
          ),
        );
      } else {
        rows.push(
          React.createElement(
            ...
            React.createElement(
              "tui-text",
              { color: isHighlighted ? highlightColor : checkBoxColor },
              `${indicator}[${checkMark}]`,
            ),
            React.createElement(
              "tui-text",
              { color: labelColor, bold },
              ` ${item.label}`,
            ),
          ),
        );
